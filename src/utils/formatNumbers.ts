...
import { Nullish, NumberType } from '@app-types/index';
...
const MAX_FRACTIONS_NO_COMMAS: NumberFormatOptions = {
...
const userInputFormatter: FormatterRule[] = [
  { customFormatter: (value: string) => value, exact: 0 },
  { formatterOptions: MAX_FRACTIONS_NO_COMMAS, upperBound: 1 },
  { customFormatter: (value: string) => value, upperBound: Infinity },
];
...
const tableDataFormatter: FormatterRule[] = [
  { exact: 0, formatterOptions: NO_DECIMALS, hardCodedInput: { hardcodedOutput: '-' } },
  {
...
const swapTradeAmountFormatter: FormatterRule[] = [
  ...
  { formatterOptions: TEN_SIG_FIGS_SIX_DECIMALS_NO_COMMAS, upperBound: Infinity },
];
...
export enum NumberFormatType {
  ...
  UserInputFormatter = 'user-input-values-formatter',
}
...
const TYPE_TO_FORMATTER_RULES = {
  ...
  [NumberFormatType.TxDisplayValuesFormatterWithSubscript]: txDisplayValuesFormatterWithSubscript,
  ...
  [NumberFormatType.UserInputFormatter]: userInputFormatter,
};
...
const getValueWithSuffixAndPrefix = (value: string, suffix: string = '', prefix: string = '') =>
  `${prefix}${value}${suffix ? ` ${suffix}` : ''}`;

export const formatNumber = ({ input, placeholder = '-', suffix, type }: FormatNumberOptions) => {
  if (
    input === null ||
    input === undefined ||
    Number.isNaN(input) ||
  ...
  const { customFormatter, formatterOptions, hardCodedInput } = getFormatterRule(
    numericInput,
    type
  );
  ...
  if (!hardCodedInput) {
    if (customFormatter) {
      ...
      if (formatterOptions)
        formattedValue = new Intl.NumberFormat(locale, formatterOptions).format(numericInput);
